...
class exactRP(initialState):
	...
	def solveAtX(self, x):		
		...
		d, p, u = self.samplePt(x)
		eint = (p/d)/(self.gamma-1.e0)
		cspd = sqrt(self.gamma*p/d)

		return d, p, u, eint, cspd


def Solve(gamma, stateL, stateR):
	rp = exactRP(gamma, stateL, stateR)

	dens, pres, velx, eint, cspd = rp.solveAtX(0.0)
